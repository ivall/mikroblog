...
def tag(tagname):
    ...
    lista = cur.fetchall()
    if lista:
        for post_id in lista:
            posty.append(post_id['post_id'])
        cur.execute("SELECT * FROM wpisy WHERE id IN %s ORDER BY `id` DESC", (posty,))
        posts = cur.fetchall()
        cur.execute("SELECT * FROM komentarze WHERE post_id IN %s ORDER BY `id` DESC", (posty,))
        comments = cur.fetchall()
